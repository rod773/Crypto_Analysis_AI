#!/usr/bin/env python3
"""High‑frequency‑trading (HFT) version of the ML‑driven predictive strategy.

Features are computed on 1‑minute bars (or any intraday interval supported by yfinance).
The model predicts the direction of the *next* bar, and a simple risk‑controlled
order is placed with tight stop‑loss / take‑profit levels (default 0.1 % / 0.2 %).

Because Yahoo Finance provides intraday data only for the most recent ~30 days,
the script limits itself to that window and uses a rolling‑training approach:
- Train on the first N days (default 5) of the fetched data.
- Test on the remaining days.
- Perform a small grid‑search over probability thresholds and risk per trade.

Usage example:
    python predictive_strategy_hft.py EURUSD=X 2024-05-01 2024-05-28 1m
"""
...
def download(ticker: str, start: str, end: str, interval: str = "1m") -> pd.DataFrame:
    # Yahoo Finance intraday data can be fetched via `period` when using minute/hour intervals.
    # If an explicit start/end is provided for an intraday interval, we fall back to using `period='30d'`.
    if interval.endswith('m') or interval.endswith('h'):
        # Use a maximum 30‑day period for intraday data
        df = yf.Ticker(ticker).history(period='30d', interval=interval, auto_adjust=False)
    else:
...
def build_features(df: pd.DataFrame) -> (pd.DataFrame, list):
    ...
    df["sma_20"] = df["adj_close"].rolling(20).mean()    # 20‑minute SMA
    df["sma_50"] = df["adj_close"].rolling(50).mean()    # 50‑minute SMA
    ...
    # Target: next‑minute up (1) vs down/no‑move (0)
...
def main() -> None:
    ...
    # Use first N days for training, rest for testing (simple split)
    split_days = 5
    ...
    # Grid‑search over thresholds and risk levels
    ...
    for high_thr in [0.60, 0.65, 0.70, 0.75]:
        for low_thr in [0.30, 0.35, 0.40, 0.45]:
            ...
            # SMA trend filter (same as before)
    ...
    if best:
        print("\n=== Best HFT grid‑search result (highest total return) ===")
